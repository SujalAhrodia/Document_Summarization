package documentsummerizationusingstackdecodermethod.decoding.stackdecoder;
...
/**
 * External source. (modified - Apoorv & Mohit)
 *
 */
public class SpecialPQ <E> implements Iterator<E>, Serializable, Cloneable {
	private static final long serialVersionUID = 3846743803859132477L;
	int size;
	int capacity;
	List<E> elements;
	double[] priorities;
	final int PQSIZE = 128;
	...
	private int getFringeIndex(int i)
	{
		int minIndex=-1;
		int parent = parent(i);
		if(priorities[leftChild(parent)] < priorities[rightChild(parent)])
			minIndex = leftChild(parent);
		else
			minIndex = rightChild(parent);
	...
	public SpecialPQ<E> clone() {
		SpecialPQ<E> clonePQ = new SpecialPQ<E>();
		clonePQ.size = size;
	...
	public SpecialPQ() {
		int newCapacity = getLegalCapacity(PQSIZE);
